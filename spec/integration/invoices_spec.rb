  ...
  describe '#update' do
    ...
    it 'updates a one-off invoice' do
      invoice = client.invoices.update(
        { payment_status: 'succeeded', metadata: [{ key: 'test', value: 'test' }] },
        invoice_id,
      )
      ...
      expect(invoice.metadata.length).to eq 1

      first_metadata = invoice.metadata.first
      expect(first_metadata.key).to eq 'test'
      expect(first_metadata.value).to eq 'test'
      expect(first_metadata.lago_id).to be_present
      expect(first_metadata.created_at).to be_present
    end
  end
end
